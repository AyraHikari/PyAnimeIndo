...
def get_main():
	...
	for anime in animelists.findAll("li"):
		...
		eps = anime.find('div', {"class": "newnime"}).text
...
def get_episodes(url):
	data = BeautifulSoup(requests.get(url).text, features="html.parser")
	ret = {'title': data.find('div', {"class": "jdlrx"}).text, 'sinopsis': "\n\n".join([x.text for x in data.find('div', {'class': 'sinopc'}).findAll('p')]), "episodes": []}
